...
exports.getAllEquipos = async (req, res) => {
    try {
        const [results] = await db.promise().query('SELECT * FROM equipos');
        res.json(results);
    } catch (err) {
...
exports.createEquipo = async (req, res) => {
    try {
        ...
        const [results] = await db.promise().query('INSERT INTO equipos SET ?', [nuevoEquipo]);
...
exports.getEquipoById = async (req, res) => {
    try {
        const { id } = req.params;
        const [results] = await db.promise().query('SELECT * FROM equipos WHERE dem = ?', [id]);
...
exports.updateEquipo = async (req, res) => {
    try {
        ...
        const [results] = await db.promise().query('UPDATE equipos SET ? WHERE dem = ?', [equipoActualizado, id]);
...
exports.deleteEquipo = async (req, res) => {
    try {
        const { id } = req.params;
        const [results] = await db.promise().query('DELETE FROM equipos WHERE dem = ?', [id]);
