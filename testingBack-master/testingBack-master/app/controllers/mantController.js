...
exports.getAllmentenimientos = async (req, res) => {
    try {
        const [results] = await db.promise().query('SELECT * FROM mantenimientos');
        res.json(results);
    } catch (err) {
...
exports.getMantenimientoById = async (req, res) => {
    try {
        ...
        const [results] = await db.promise().query('SELECT * FROM mantenimientos WHERE id = ?', [id]);
...
exports.updateMantenimientoById = async (req, res) => {
    try {
        ...
        await db.promise().query('UPDATE mantenimientos SET nombre = ?, empresa = ?, numero = ?, desde = ?,hasta = ? WHERE id = ?', [nombre, empresa, numero,desde,hasta, id]);
...
exports.deleteMantenimiento = async (req, res) => {
    try {
        const { id } = req.params;
        const [results] = await db.promise().query('DELETE FROM mantenimientos WHERE id = ?', [id]);
...
exports.getMantenimientosBeforeDate = async (req, res) => {
    try {
        ...
        const [results] = await db.promise().query('SELECT * FROM mantenimientos WHERE hasta < ?', [hasta]);
